...
def load_dataset(data_path, dic_path):
	dataset = ArxivClassify(data_path,maxlen=args.maxlen)
	return dataset
...
def evaluate(model,elmo,dataset,device):
...
def train(model,elmo,train_dataset,device='cuda',val_dataset=None):
	...
	try:
		for epoch in range(1, args.epochs+1):
			...
			train_loss,train_acc = train_epoch(model,elmo,train_dataset,epoch=epoch,device=device,lr=lr)
...
def main():
	...
	print('val loaded')
	model, elmo = load_model(path_to_glove,train_dataset.word_to_index)
	model = model.to(device=device)
	elmo = elmo.to(device=device)
	print('model loaded')
	# print(model)
	train(model,elmo,train_dataset,device,val_dataset)
